...
class Net(network.resnet38d.Net):
    def __init__(self):
        ...
        # teacher_pre = torch.load(args.teacher_model)
        # teacher_dict = teacher.state_dict()
        # teacher_dict.update(teacher_pre)
        # teacher.load_state_dict(teacher_dict)
        # print('Loading pretrained teacher model weights from {:s}'.format(args.teacher_model))
        # teacher.eval()
        # teacher.cuda()
    ...
    def patch_based_metric_loss(self, x_patch, label, patch_num=4, is_same_img=False, is_hard_negative=True):
        ...
        for i in range(N_f):# for 每张图片
            ...
            if len(label_list) > 0:   # 用cams激活区域的boundingbox去框在最后一组特征（256维）中的对应位置，然后切成patch_num份，每一份中每一维做一个全局平均池化
                ...
                roi_cls_label.extend(list(label_list) * patch_num)    # [1,2,3]*3=[1, 2, 3, 1, 2, 3, 1, 2, 3]

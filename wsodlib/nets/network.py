...
class Network(nn.Module):
    ...
    def _anchor_component(self, height, width):
        ...
        anchors, anchor_length = generate_anchors_pre( \
            height, width,
            self._feat_stride, self._anchor_scales, self._anchor_ratios)   # anchors:shape:(67500, 4); anchor_length:67500
    ...
    def _smooth_l1_loss(self, bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
        ...
        smoothL1_sign = (abs_in_box_diff < 1. / sigma_2).detach().float()  # 找出差异小于1的预测结果
        in_loss_box = torch.pow(in_box_diff, 2) * (sigma_2 / 2.) * smoothL1_sign \
                      + (abs_in_box_diff - (0.5 / sigma_2)) * (1. - smoothL1_sign)  # 差异（绝对值）小于1的直接平方；差异大于1的直接取差异绝对值-0.5的结果
    ...
    def _region_proposal(self, net_conv):
        self._anchor_component(net_conv.size(2), net_conv.size(3))     # net_conv:[1,152,75,100] 数据经过backbone后得到的结果
        rpn = F.relu(self.rpn_net(net_conv))               # rpn:[1,152,75,100]
        ...
        rpn_cls_score = rpn_cls_score.permute(0, 2, 3, 1)  # batch * h * w * (num_anchors * 2)         # final format
        ...
        rpn_bbox_pred = rpn_bbox_pred.permute(0, 2, 3, 1).contiguous()  # batch * h * w * (num_anchors*4)         # final format

        if self._mode == 'TRAIN':

            rois, roi_scores = self._proposal_layer(rpn_cls_prob, rpn_bbox_pred)  # rois, roi_scores are varible     # NMS后剩2000个rois，每个roi对应5个值

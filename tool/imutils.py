...
def reNormalize(imgs):
    ...
    imgs[:, :, :, 0] = (imgs[:, :, :, 0] * std[0] + mean[0]) * 255
    imgs[:, :, :, 1] = (imgs[:, :, :, 1] * std[1] + mean[1]) * 255
    imgs[:, :, :, 2] = (imgs[:, :, :, 2] * std[2] + mean[2]) * 255
    imgs[imgs > 255] = 255
    imgs[imgs < 0] = 0
...
def crf_inference(img, probs, t=10, scale_factor=1, labels=21):
    ...
    # d.addPairwiseBilateral(sxy=70/scale_factor, srgb=13, rgbim=np.copy(img), compat=10)

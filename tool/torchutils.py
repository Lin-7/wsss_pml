...
class Adam(torch.optim.SGD):
    def __init__(self, params, lr, weight_decay, max_step, adam_turn=0,momentum=0.9, global_step=0):
        super().__init__(params, lr, weight_decay=weight_decay)
        self.global_step = global_step
        self.max_step = max_step
        self.adam_turn=adam_turn
        #self.step_per_epoch=int(max_step/num_epoch)
    ...
    def adam_turn_step(self):
        lr_mult = 0.5

        for i in range(len(self.param_groups)):
            self.param_groups[i]['lr'] = self.param_groups[i]['lr'] * lr_mult

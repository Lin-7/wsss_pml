...
if __name__ == '__main__':
    ...
    # 机器和环境的不同，会差一两个点
    parser.add_argument("--batch_size", default=3, type=int)   # 10/12   一个gpu：8×6√  两个gpu：10
    parser.add_argument("--max_epoches", default=4, type=int)   # 根据机器去修改
    parser.add_argument("--network", default="network.resnet38_cls_moco", type=str)
    parser.add_argument("--lr", default=0.01, type=float)
    parser.add_argument("--num_workers", default=2, type=int)         # 之前是8，用了ddp后改为4
    parser.add_argument("--num_workers_infer", default=2, type=int)   # torch.utils.data.dataloader提示建议创建12个workers
    ...
    parser.add_argument("--using_momentummodel", default=False, type=bool)
    parser.add_argument("--model_update_m", default=0.7, type=float)
    parser.add_argument("--is_hard_negative", default=True, type=bool)
    parser.add_argument("--bghard", default=False, type=bool)
    parser.add_argument("--patch_loss_weight", default=0.01, type=float)

    parser.add_argument("--session_name", default="0514-onlyclsloss", type=str)         # train val test
    ...
    if args.local_rank == 0:
        # 复制主要运行文件
        copy_files_list = ['/usr/volume/WSSS/WSSS_PML/train_cls_loc_jointly_new_ddp.py', 
    ...
    # tensorboard_sampler = DistributedSampler(tensorboard_dataset)
    tensorboard_sampler = DistributedSampler(tensorboard_dataset, num_replicas=args.world_size, rank=args.local_rank)
    tensorboard_img_loader = DataLoader(tensorboard_dataset, sampler=tensorboard_sampler, 
                                        shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    ...
    infer_sampler = DistributedSampler(infer_dataset, num_replicas=args.world_size, rank=args.local_rank)
    infer_data_loader = DataLoader(infer_dataset, shuffle=False, sampler=infer_sampler, 
                                   num_workers=args.num_workers_infer, pin_memory=True)
    ...
    for ep in range(args.max_epoches):
        ...
        '''
        tensorboard_sampler.set_epoch(ep)
